...
use crate::utils::{associate_convert_token_account, create_ata_if_needed, is_ata, is_token_account_initialized, log_claim_info_after, log_claim_info_before, transfer_sol, transfer_token};
use crate::*;
...
pub fn claim_handler<'a>(
    ctx: Context<'_, '_, 'a, 'a, ClaimAccounts<'a>>,
) -> Result<()> {
    let destination_account = ctx.accounts.destination_token_account.clone();

    if destination_account.is_some() && !is_token_account_initialized(&destination_account.as_ref().unwrap()) {
        ...
        require!(ctx.accounts.associated_token_program.is_some(), ErrorCode::InvalidAssociatedTokenProgram);
    ...
    // Handle SOL claim
    if destination_account.is_some() && is_ata(&ctx.accounts.destination_token_account.as_ref().unwrap()) {
...
fn handle_sol_claim<'a>(ctx: &Context<'_, '_, 'a, 'a, ClaimAccounts<'a>>) -> Result<()> {
    require!(ctx.accounts.sa_authority.lamports() > MIN_SOL_ACCOUNT_RENT, ErrorCode::InsufficientFunds);


    let amount = ctx.accounts.sa_authority
        .lamports()
    ...
    log_claim_info_before(
        before_authority_balance, 
    ...
    let authority_sol_change = before_authority_balance.checked_sub(after_authority_balance).ok_or(ErrorCode::CalculationError)?;
    let receiver_sol_change = after_receiver_balance.checked_sub(before_receiver_balance).ok_or(ErrorCode::CalculationError)?;
...
fn handle_token_claim<'a>(ctx: &Context<'_, '_, 'a, 'a, ClaimAccounts<'a>>) -> Result<()> {
    ...
    let mut destination_token_account = associate_convert_token_account(&ctx.accounts.destination_token_account.as_ref().unwrap())
    ...
    require!(destination_token_account.owner == ctx.accounts.receiver.key(), ErrorCode::InvalidDestinationTokenAccount);
    require!(source_token_account.amount > 0, ErrorCode::InsufficientFunds);
    require!(source_token_account.mint == destination_token_account.mint, ErrorCode::InvalidTokenMint);
