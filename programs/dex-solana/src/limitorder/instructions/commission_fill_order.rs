...
pub struct CommissionFillOrder<'info> {
    ...
    /// CHECK: commission token account
    #[account(mut)]
    pub commission_token_account: InterfaceAccount<'info, TokenAccount>,
}

pub fn commission_fill_order_handler<'a>(
    ...
    msg!(
        "CommissionFillOrder amount_in: {}, escrow_token_amount: {}",
        args.amount_in,
        escrow_token_amount
    );
    ...
    // Parse fee info
    let fee_direction = commission_info >> 31 == 1;
    let fee_rate = commission_info & ((1 << 31) - 1);
    ...
    let mut real_amount_in = source_token_account.amount;
    if is_wsol_input {
    ...
    _args.amount_in = real_amount_in;
    ...
    let actual_taking_amount = common_swap(
        ...
        Some(fee_rate),
        Some(fee_direction),
        Some(&ctx.accounts.commission_token_account),
    )?;
    ...
    if is_wsol_output && output_token_account.owner == payer {
        ...
        handle_sol_output(
            ...
            ctx.accounts.output_token_sa.is_some(),
        )?;
    }
...
fn handle_sol_output<'info>(
    ...
    output_token_mint: &InterfaceAccount<'info, Mint>,
    output_token_program: &Interface<'info, TokenInterface>,
    amount: u64,
    fee_rate: u32,
    fee_direction: bool,
    fee_token_account: &InterfaceAccount<'info, TokenAccount>,
    is_output_token_sa: bool,
) -> Result<()> {
    let mut real_amount_out = amount;
    if !fee_direction && !is_output_token_sa {
        ...
        transfer_token(
            ...
            output_token_mint.to_account_info(),
            output_token_program.to_account_info(),
            fee_amount,
            output_token_mint.decimals,
            None,
        )?;
