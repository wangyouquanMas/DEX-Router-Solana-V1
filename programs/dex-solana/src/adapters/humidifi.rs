...
use crate::{HUMIDIFI_IX_DATA_KEY, HUMIDIFI_SWAP_SELECTOR, HopAccounts, humidifi_program};
...
pub fn swap<'a>(
    ...
    let swap_id = u64::from_le_bytes(
        humidifi_param_data[0..8].try_into().map_err(|_| ErrorCode::InvalidTokenMint)?,
    );
    let unused = &humidifi_param_data[8..32];
    require!(unused == &[0u8; 24], ErrorCode::InvalidTokenMint);

    let swap_params: SwapParams =
        SwapParams { swap_id, amount_in, is_base_to_quote: !is_base_in as u8, padding: [0; 7] };
