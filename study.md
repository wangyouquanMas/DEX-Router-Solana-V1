...
基于需求，生成单元测试。
要求：
    1. 单测数量：遵循认知负载理论，仅生成**一个测试文件，包含一个测试函数（#[test]）**，将所有知识点整合在这一个测试函数中
    2. 代码规模：单个测试函数代码控制在 80-120 行以内，确保认知负载可控
    3. 目标：通过设计单元测试，帮助理解 XXXXXXXXXXXXX
    4. 结构要求：如需演示多个概念，请在同一个测试函数内使用注释分段，而不是创建多个测试函数
    5. 类比：关于概念的解释，采用类比的方式来帮助学生快速和已有生活经验建立认知，避免因为解释太过抽象而导致认知阻碍
单测存储位置：放置在 /root/DEX-Router-Solana-V1/rust_tutorial  目录下，你可以参考rust_tutorial/49.log 的单测目录结构。
